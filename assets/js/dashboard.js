...
const results = await Promise.all([
  supabase.from('mentions').select('*',{count:'exact',head:true}).gte('published_at',today.toISOString()),
  supabase.from('mentions').select('*',{count:'exact',head:true}).gte('published_at',today.toISOString()).gte('priority_score',81),
  supabase.from('mentions').select('*',{count:'exact',head:true}).gte('published_at',today.toISOString()).eq('sentiment','negative'),
  supabase.from('mentions').select('*',{count:'exact',head:true}).gte('published_at',today.toISOString()).eq('sentiment','positive'),
  supabase.from('mentions').select('id,title,summary,source_platform,source_url,priority_score,sentiment,detected_at,published_at,raw_payload,mention_media(url,media_type)').order('published_at',{ascending:false,nullsFirst:false}).limit(6),
...
if(notifIds.length){
  const {data:linked=[]}=await supabase.from('mentions').select('id,published_at,raw_payload').in('id',notifIds);
  mentionMap=new Map(linked.map(x=>[x.id,x]));
}
notifRows=notifRows.sort((a,b)=>new Date(mentionMap.get(b.mention_id)?.published_at||b.created_at||0)-new Date(mentionMap.get(a.mention_id)?.published_at||a.created_at||0)).slice(0,5);
n.innerHTML=notifRows.length?notifRows.map(x=>{const m=mentionMap.get(x.mention_id);const comment=isComment(m);return `<a class="dashboard-notification${comment?' is-comment':''}" href="${x.mention_id ? `./monitorinq.html?id=${x.mention_id}` : './bildirisler.html'}">${comment?commentMark():''}<span class="notification-dot ${x.kind==='critical'?'critical':'system'}"></span><span><strong>${escapeHtml(x.title||'Bildiriş')}</strong><small>${escapeHtml(x.body||'')}</small></span><time>${fmtDate(m?.published_at||x.created_at)}</time></a>`}).join(''):'<div class="empty compact-empty">Yeni bildiriş yoxdur.</div>';
