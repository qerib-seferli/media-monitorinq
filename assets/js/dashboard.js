...
async function fetchDashboardData(){
  ...
  const results=await Promise.all([
    scoped(supabase.from('mentions').select('*',{count:'exact',head:true}).gt('relevance_score',0).gte('published_at',today.toISOString())),
    scoped(supabase.from('mentions').select('*',{count:'exact',head:true}).gt('relevance_score',0).gte('published_at',today.toISOString()).gte('priority_score',81)),
    scoped(supabase.from('mentions').select('*',{count:'exact',head:true}).gt('relevance_score',0).gte('published_at',today.toISOString()).eq('sentiment','negative')),
    scoped(supabase.from('mentions').select('*',{count:'exact',head:true}).gt('relevance_score',0).gte('published_at',today.toISOString()).eq('sentiment','positive')),
    scoped(supabase.from('mentions').select('id,title,summary,source_platform,source_url,priority_score,sentiment,detected_at,published_at,source_status,raw_payload,organization_id,mention_media(url,media_type)').gt('relevance_score',0).order('published_at',{ascending:false,nullsFirst:false}).limit(6)),
    scoped(supabase.from('notifications').select('*').order('created_at',{ascending:false}).limit(30))
  ]);
  const fatal=results.find(x=>x.error); if(fatal?.error) console.warn(friendlyError(fatal.error));
  return results;
}
...
async function renderDashboard(){
  ...
  const [{count:total},{count:critical},{count:negative},{count:positive},{data:latestData},{data:notifData}] = results;
  latest=latestData||[]; notifs=notifData||[];
...
if(notifIds.length){
  const {data:linked=[]}=await supabase.from('mentions').select('id,published_at,raw_payload,relevance_score,source_status,title,author_name').in('id',notifIds);
