...
n.innerHTML=notifRows.length?notifRows.map(x=>{const m=mentionMap.get(x.mention_id);const comment=isComment(m);return `<a class="dashboard-notification${comment?' is-comment':''}" href="${x.mention_id ? `./monitorinq.html?id=${x.mention_id}` : './bildirisler.html'}"><span class="notification-dot ${x.kind==='critical'?'critical':'system'}">${comment?'✉':''}</span><span><strong>${escapeHtml(x.title||'Bildiriş')}</strong><small>${escapeHtml(x.body||'')}</small></span><time>${fmtDate(m?.published_at||x.created_at)}</time></a>`}).join(''):'<div class="empty compact-empty">Yeni bildiriş yoxdur.</div>';
