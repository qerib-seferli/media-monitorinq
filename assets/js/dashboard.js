...
async function fetchDashboardData(){
  ...
  const countQuery=(extra=(q)=>q)=>extra(scoped(
    supabase.from('mentions').select('id',{count:'exact',head:true})
      .gte('relevance_score',31).gte('detected_at',todayIso)
  ));
...
notifRows=notifRows.filter(x=>{if(!x.mention_id)return true;const m=mentionMap.get(x.mention_id);return Number(m?.relevance_score||0)>=31&&!isMentionExcluded(m,results.excludes);}).sort((a,b)=>new Date(b.created_at||0)-new Date(a.created_at||0)).slice(0,8);
