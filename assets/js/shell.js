...
export function renderShell(profile, active='dashboard') {
  ...
  if (topbar) topbar.innerHTML = `<div class="topbar-left"><img class="state-emblem" src="./assets/img/state-emblem.svg" alt="AZ"><span class="muted" style="font-size:11px">Rəqəmsal Media<br>Monitorinq</span></div><div class="topbar-center"><strong>${orgName}</strong><span>Rəqəmsal Media Monitorinq Sistemi</span></div><div class="profile-chip"><div class="profile-text"><strong>${profile.first_name||''} ${profile.last_name||''}</strong><span>${profile.positions?.name || (isAdmin?'Super Administrator':'İstifadəçi')}</span></div><div class="avatar">${avatarText(profile)}</div></div>`;
