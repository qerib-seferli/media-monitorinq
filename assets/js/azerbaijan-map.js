  ...
  for(let from=0;from<20000;from+=1000){
    ...
    for(const m of rows){
      ...
      if(m.district_id){
        ...
        const uKey=`${m.organization_id}|${platform}`;
        unscopedCounts.set(uKey,(unscopedCounts.get(uKey)||0)+1);
        if(!unscopedLatest.has(String(m.organization_id)))unscopedLatest.set(String(m.organization_id),m.detected_at);
      }
    }
    if(rows.length<1000)break;
  }
  return {orgs,districts,servicePoints,serviceAreas,counts,districtCounts,unscopedCounts,latest,districtLatest,unscopedLatest};
}
...
function servicePointTypeLabel(point={}){const name=String(point?.name||'');if(/Sukanal İdarəsi/i.test(name))return 'Şəhər idarəsi';if(/Şöbəsi/i.test(name))return 'Tabeli şöbə';return 'Tabeli təşkilati vahid';}
...
export async function initAzerbaijanMonitoringMap({rootId='azerbaijan-live-map',profile=null,allowScan=false,serverSync=true}={}){
  ...
  const {orgs,districts,servicePoints,serviceAreas,counts,districtCounts,unscopedCounts,latest,districtLatest,unscopedLatest}=data;
  ...
  function isPhysicallyInDistrict(o,districtId){
    if(!o||!districtId)return false;
    return String(o.location_district_id||'')===String(districtId);
  }
  function orgSources(o,districtId=null,{placeUnscopedAtPhysical=false}={}){
    const names=['Web','YouTube','Facebook','Instagram','TikTok','Telegram','RSS'];
    return names.map(name=>{
      let n=districtId?(districtCounts.get(`${o.id}|${districtId}|${name}`)||0):(counts.get(`${o.id}|${name}`)||0);
      // Rayon etiketi olmayan nəticə yalnız təşkilatın faktiki yerləşdiyi ərazidə göstərilir.
      // Beləliklə mərkəzi qurumun ümumi nəticəsi bütün xidmət rayonlarına yayılmır.
      if(districtId&&placeUnscopedAtPhysical&&isPhysicallyInDistrict(o,districtId))n+=unscopedCounts.get(`${o.id}|${name}`)||0;
      return [name,n];
    }).filter(([,n])=>n>0);
  }
  function sourceTotal(rows=[]){return rows.reduce((sum,[,n])=>sum+Number(n||0),0);}
  function orgLatest(o,districtId=null,{placeUnscopedAtPhysical=false}={}){
    if(!districtId)return latest.get(String(o.id));
    const scoped=districtLatest.get(`${o.id}|${districtId}`);
    const unscoped=placeUnscopedAtPhysical&&isPhysicallyInDistrict(o,districtId)?unscopedLatest.get(String(o.id)):null;
    if(!scoped)return unscoped;if(!unscoped)return scoped;
    return new Date(scoped)>=new Date(unscoped)?scoped:unscoped;
  }
  function physicalOrganizations(districtId){if(!districtId)return [];return activeOrgs.filter(o=>isPhysicallyInDistrict(o,districtId));}
  function physicalServicePoints(districtId){if(!districtId)return [];return servicePoints.filter(p=>p.is_active!==false&&String(p.district_id||'')===String(districtId));}
  function physicalUnitCount(districtId){return physicalOrganizations(districtId).length+physicalServicePoints(districtId).length;}
  function renderDetail(path){
    ...
    const total=activeOrgs.reduce((sum,o)=>sum+sourceTotal(orgSources(o,districtId,{placeUnscopedAtPhysical:true})),0);
    const orgCards=rows.map((o,index)=>{
      ...
      const localSources=orgSources(o,districtId,{placeUnscopedAtPhysical:true});
      ...
      const lastSeen=localTotal>0?orgLatest(o,districtId,{placeUnscopedAtPhysical:true}):orgLatest(o);
    ...
    const pointCards=points.map(p=>{const parent=activeOrgById.get(String(p.organization_id));const stateKey=detailStateKey(districtId,'point',String(p.id));const isOpen=detailOpenState.get(stateKey)===true;const manager=String(p.manager_name||'').trim();const managerTitle=String(p.manager_title||'').trim();return `<details class="az-org-row az-service-point az-org-collapsible hierarchy-level-4" data-detail-state-key="${escapeHtml(stateKey)}" ${isOpen?'open':''}><summary class="az-org-summary"><span class="az-hierarchy-mark" aria-hidden="true"></span><span class="az-org-summary-main"><strong>${escapeHtml(p.name||'Tabeli təşkilati vahid')}</strong><small>${escapeHtml(servicePointTypeLabel(p))}${parent?` · ${escapeHtml(parent.short_name||parent.name||'')}`:''}</small></span><span class="az-collapse-chevron" aria-hidden="true">⌄</span></summary><div class="az-org-collapsible-body">${parent?`<div class="az-point-parent">Tabeli qurum: <b>${escapeHtml(parent.short_name||parent.name||'')}</b></div>`:''}${manager?`<div class="az-point-manager">${escapeHtml(managerTitle||'Rəhbər')}: <b>${escapeHtml(manager)}</b></div>`:''}<small class="az-org-address">${escapeHtml(p.address_text||'Rəsmi ünvan bazaya daxil edilməyib')}</small>${contactLinks(p)}${sourceLink(p)}</div></details>`;}).join('');
    const physicalCount=physicalOrganizations(districtId).length;
    const physicalPointCount=physicalServicePoints(districtId).length;
    const physicalUnits=physicalCount+physicalPointCount;
    const districtResultText=total>0?`${total} məlumat`:'ərazi üzrə nəticə yoxdur';
    const coverageText=rows.length!==physicalCount?` · ${rows.length} xidmət göstərən`:'';
    detail.innerHTML=`<div class="az-map-detail-head"><div><span class="eyebrow">Seçilən ərazi</span><h3>${escapeHtml(district?.name||mapName||'Ərazi')}</h3><p>${physicalUnits} fiziki təşkilati vahid${coverageText}${physicalPointCount?` · ${physicalPointCount} tabeli vahid`:''} · ${districtResultText}${hit?` · <b>son skanda +${hit}</b>`:''}</p></div></div>${rows.length?`<div class="az-detail-section-title">Xidmət göstərən təşkilatlar <b>${rows.length}</b></div><div class="az-org-list">${orgCards}</div>`:(points.length?'<div class="empty compact">Bu ərazi ayrıca əsas təşkilat deyil, tabeli idarə/şöbə vasitəsilə əhatə olunur.</div>':'<div class="empty compact">Bu əraziyə aktiv xidmət əhatəsi təyin edilməyib.</div>')}${points.length?`<div class="az-detail-section-title service">Tabeli idarə və şöbələr <b>${points.length}</b></div><div class="az-org-list">${pointCards}</div>`:''}`;
  ...
  function showTooltip(path,event){
    ...
    const physicalCount=district?physicalUnitCount(String(district.id)):0;tooltip.innerHTML=`<strong>${escapeHtml(district?.name||mapName)}</strong><span>${physicalCount} fiziki təşkilati vahid${rows.length?` · ${rows.length} əsas xidmət göstərən`:''}${hit?` · son skan +${hit}`:''}</span>`;tooltip.hidden=false;
  ...
  function refreshVisuals(){
    ...
    for(const path of paths){
      const {district,rows}=districtInfo(path);const mapDistrictId=district?String(district.id):null;const total=activeOrgs.reduce((sum,o)=>sum+sourceTotal(orgSources(o,mapDistrictId,{placeUnscopedAtPhysical:true})),0);
