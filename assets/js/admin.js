...
function serviceAreaIdsForOrg(org) {
  const selected = organizationServiceAreas
    .filter(x => String(x.organization_id) === String(org?.id))
    .map(x => String(x.district_id));
  if (selected.length) return [...new Set(selected)];
  return org?.district_id ? [String(org.district_id)] : [];
}

function serviceAreaNamesForOrg(org) {
  const ids = serviceAreaIdsForOrg(org);
  return ids.map(id => districts.find(d => String(d.id) === String(id))?.name).filter(Boolean);
}

function serviceAreaSummaryForOrg(org) {
  const names = serviceAreaNamesForOrg(org);
  if (!names.length) return 'Təyin edilməyib';
  if (names.length === 1) return names[0];
  return `${names.length} ərazi`;
}

function serviceAreaTitleForOrg(org) {
  const names = serviceAreaNamesForOrg(org);
...
async function deleteServicePoint(id) {
  const point = organizationServicePoints.find(x => String(x.id) === String(id));
  if (!point) return;
  const ok = await confirmDialog({title:'Tabeli vahid silinsin?',message:`${point.name || 'Seçilmiş tabeli vahid'} reyestrdən silinəcək. Parent təşkilat və onun monitorinq nəticələri silinməyəcək.`,confirmText:'Bəli, sil',cancelText:'Xeyr',tone:'danger'});
  if (!ok) return;
  const { error } = await supabase.from('organization_service_points').delete().eq('id', id);
  toast(error ? error.message : 'Tabeli vahid silindi.', error ? 'error' : 'success');
  if (!error) await refresh();
}
...
function serviceAreaPickerHtml(editing) {
  ...
  return `<details class="service-area-details">
    <summary><span>Xidmət əraziləri / monitorinq əhatəsi</span><span class="service-area-summary-count"><b id="org-service-area-count">${selectedIds.size}</b> seçilib</span></summary>
    <div class="service-area-details-body">
      <div class="service-area-tools">
        <span class="field-help">Yalnız təşkilatın real xidmət etdiyi əraziləri seç. Əhatə dəyişərsə qutuları sonradan yenilə.</span>
        <div class="service-area-actions"><button type="button" class="btn ghost btn-sm" id="org-service-area-clear">Seçimi təmizlə</button></div>
      </div>
...
async function saveServicePoint(e) {
  e.preventDefault();
  ...
  const row={
    ...
    manager_title:document.querySelector('#point-manager-title').value.trim()||null,
    manager_name:document.querySelector('#point-manager-name').value.trim()||null,
    address_text:document.querySelector('#point-address').value.trim()||null,
    phone:document.querySelector('#point-phone').value.trim()||null,
    email:document.querySelector('#point-email').value.trim().toLowerCase()||null,
    official_source_url:document.querySelector('#point-source').value.trim()||null,
    is_active:document.querySelector('#point-active').checked,
...
async function saveOrg(e) {
  ...
  const selectedServiceAreas = [...document.querySelectorAll('#org-service-areas input:checked')].map(x => x.value);
  const existingOrg = orgId ? orgs.find(o => String(o.id) === String(orgId)) : null;
  const legacyPrimaryDistrict = existingOrg?.district_id && selectedServiceAreas.includes(String(existingOrg.district_id))
    ? existingOrg.district_id
  ...
  const row = {
    ...
    district_id: legacyPrimaryDistrict,
    ...
    phone: document.querySelector('#org-phone')?.value.trim() || null,
    email: document.querySelector('#org-email')?.value.trim().toLowerCase() || null,
  ...
  let error = null;
  if (orgId) {
    ({ error } = await supabase.from('organizations').update(row).eq('id', orgId));
  } else {
    const result = await supabase.from('organizations').insert(row).select('id').single();
    error = result.error;
    savedOrgId = result.data?.id || '';
  }
  if (!error && savedOrgId) {
    const { error: deleteError } = await supabase.from('organization_service_areas').delete().eq('organization_id', savedOrgId);
    if (deleteError) error = deleteError;
    if (!error && selectedServiceAreas.length) {
      const { error: areaError } = await supabase.from('organization_service_areas').insert(
        selectedServiceAreas.map(district_id => ({ organization_id: savedOrgId, district_id }))
      );
      if (areaError) error = areaError;
    }
  }
  toast(error ? error.message : (orgId ? 'Təşkilat və xidmət əraziləri yeniləndi' : 'Təşkilat yaradıldı'), error ? 'error' : 'success');
  if (!error) { closeModal(); await refresh(); location.hash = 'organizations'; route(); }
}
